...
export default function AdminAnalyticsPage() {
  ...
  const analytics = useMemo((): AnalyticsData => {
    ...
    const todayRecords = records.filter(
      (r: any) =>
        new Date(r.tanggalSetor).toDateString() === today.toDateString()
    );
    const weeklyRecords = records.filter((r: any) =>
      isWithinInterval(new Date(r.tanggalSetor), { start: weekAgo, end: today })
    );
    ...
    const avgCompletionRate =
      filteredRecords.length > 0
        ? Math.round((completedKaca / filteredRecords.length) * 100)
        : 0;
    ...
    const monthlyMap = new Map<
      string,
      { completed: number; new: number; recheck: number }
    >();
    ...
    const monthlyProgress = Array.from(monthlyMap.entries())
      ...
      .slice(-6)
      .map(([month, data]) => ({
        month: format(new Date(month + "-01"), "MMM", { locale: idLocale }),
        ...data,
      }));
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayRecords = records.filter(
        (r: any) =>
          new Date(r.tanggalSetor).toDateString() === date.toDateString()
      );
      weeklyActivity.push({
        day: format(date, "EEE", { locale: idLocale }),
        hafalan: dayRecords.length,
        recheck: dayRecords.filter(
          (r: any) => r.statusKaca === "RECHECK_PASSED"
        ).length,
      });
    }
    ...
    const teacherMap = new Map<
      string,
      { name: string; completedCount: number; totalRecords: number }
    >();
    filteredRecords.forEach((r: any) => {
    ...
    const teacherPerformance = Array.from(teacherMap.values())
      .map((t) => ({
        ...
        efficiency:
          t.totalRecords > 0
            ? Math.round((t.completedCount / t.totalRecords) * 100)
            : 0,
      }))
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Analisis komprehensif sistem hafalan Al-Qur&apos;an
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="overview" className="space-y-6 mt-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <StatsCard
                title="Total Hafalan"
                value={analytics.hafalanStats.totalRecords}
                description={`${analytics.hafalanStats.todayRecords} hari ini`}
              ...
              <StatsCard
                title="Lulus Recheck"
                value={analytics.hafalanStats.completedKaca}
                description={`${analytics.hafalanStats.avgCompletionRate}% completion rate`}
              ...
              <StatsCard
                ...
                value={analytics.hafalanStats.waitingRecheck}
              ...
              <StatsCard
                title="Sedang Proses"
                value={analytics.hafalanStats.inProgress}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>
                    Persentase hafalan yang telah lulus recheck
                  </CardDescription>
                </CardHeader>
                <CardContent className="flex justify-center py-6">
                  <ProgressRing
                    value={analytics.hafalanStats.avgCompletionRate}
                    size={160}
                    strokeWidth={12}
                    color={CHART_COLORS.primary}
                  />
              ...
              <Card className="lg:col-span-2">
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>
                    Pembagian status seluruh hafalan dalam periode terpilih
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {pieData.length > 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>
                    Tren hafalan baru vs yang lulus per bulan
                  </CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>
                    Hafalan dan recheck 7 hari terakhir
                  </CardDescription>
                </CardHeader>
                <CardContent>
          ...
          <TabsContent value="users" className="space-y-6 mt-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              <StatsCard
                ...
                description={`${analytics.userStats.newThisMonth} baru bulan ini`}
              ...
              <StatsCard
                title="Santri"
                value={analytics.userStats.totalSantri}
              ...
              <StatsCard
                title="Guru"
                value={analytics.userStats.totalTeachers}
              ...
              <StatsCard
                title="Wali"
                value={analytics.userStats.totalWalis}
                icon={<Users className="h-5 w-5" />}
                color="info"
                loading={loading}
              />
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Perbandingan jumlah pengguna berdasarkan role
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <TabsContent value="performance" className="space-y-6 mt-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Berdasarkan jumlah hafalan yang dibimbing
                  </CardDescription>
                </CardHeader>
                <CardContent>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 text-sm text-gray-500">
                              <span>{teacher.completedCount} lulus</span>
                              <span>•</span>
                              <span>{teacher.santriCount} total</span>
                            </div>
                          </div>
                          <Badge
                            variant="outline"
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Berdasarkan jumlah hafalan yang lulus recheck
                  </CardDescription>
                </CardHeader>
                <CardContent>
                      ...
                      {analytics.topSantri.map((santri, index) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 text-sm text-gray-500">
                              <span>{santri.completedKaca} kaca lulus</span>
                              <span>•</span>
                              <span>Terakhir: {santri.lastActivity}</span>
                            </div>
                          </div>
                          <Badge
                            variant="outline"
            ...
            <Card>
              <CardHeader>
                <CardTitle>Distribusi per Juz</CardTitle>
                <CardDescription>
                  Jumlah hafalan berdasarkan juz yang sedang dipelajari
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <TabsContent value="activity" className="space-y-6 mt-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <StatsCard
                ...
                value={analytics.hafalanStats.todayRecords}
                icon={<Calendar className="h-5 w-5" />}
                color="primary"
                loading={loading}
              />
              <StatsCard
                title="Hafalan Minggu Ini"
                value={analytics.hafalanStats.weeklyRecords}
                icon={<Activity className="h-5 w-5" />}
                color="info"
                loading={loading}
              />
              <StatsCard
                title="Total Periode Ini"
                value={analytics.hafalanStats.totalRecords}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Aktivitas hafalan dalam 7 hari terakhir
                </CardDescription>
              </CardHeader>
              <CardContent>
